...
let body t = let tt z = point_of_cx (apply_hyp_iso t z)
  in make_sketch [Seg (List.map tt [p0;p1;p2;p3;p4;p5;p6;p7;p8;p9;
                                    p10;p11;p12;p13;p14;p15;p16;p17;
                                    p18;p19;p0])] ;;

(** +quad+ *)
let quad t = let tt z = point_of_cx (apply_hyp_iso t z) in
  make_sketch [Seg (List.map tt [ptA;ptB;ptD;ptC;ptA])] ;;

(** +dessin+ *)
let drawing t = let tt z = point_of_cx (apply_hyp_iso t z)
  in
  group_sketches [
    make_sketch [Seg (List.map tt [p0;q1;q2;q3;p10])];
    make_sketch [Seg (List.map tt [q4;q5])];
    make_sketch [Seg (List.map tt [q6;q7])];
    make_sketch [Seg (List.map tt [q8;q9;q10;q11])];
    make_sketch [Seg (List.map tt [q12;q13;q14;q12])];
    make_sketch [Seg (List.map tt [q15;q16;q17;q15])];
    make_sketch [Seg (List.map tt [r1;r2;r3])];
    make_sketch [Seg (List.map tt [r4;r5;r6])];
    make_sketch [Seg (List.map tt [r7;r8;r9])];
    make_sketch [Seg (List.map tt [r10;r11])];
    make_sketch [Seg (List.map tt [r12;r13])];
    make_sketch [Seg (List.map tt [q9;r14])];
    make_sketch [Seg (List.map tt [q10;r15] )]
  ] ;;
...
let model_fish =
  ...
  group_pictures
    ([ transform_picture t
       ...
       west c 10.0 (text "C") ;
       north d 10.0 (text "D") ]
     @ (List.map (center_picture bullet) [ a; b; c ])) ;;
...
let escher_transfo =
  let transfo = function
    ...
    | Tc -> hyp_rotation ptC (-.90.0)
  in (fun (ct, tl) ->
      (ct, List.fold_left (fun t t' -> compose_hyp_iso t (transfo t'))
         hyp_identity tl)) ;;
...
let simplify_escher d =
  select (fun (_, x) -> modul (apply_hyp_iso x center) < d) ;;

(** +make_escher_tiling+ *)
let make_escher_tiling n d =
  group_pictures
    (List.map (fun (t1, t2) -> (fish t2 (t1.(0))))
       (simplify_escher d
          (List.map escher_transfo
