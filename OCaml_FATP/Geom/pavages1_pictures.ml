...
let tile1_decor=
  group_pictures
    ([tile1; south ptA1 10.0 (text "A"); south ptB1 10.0 (text "B");
     ...
     @ (List.map (center_picture bullet) [ptA1; ptB1; ptC1; ptD1; ptE1; ptF1])) ;;
...
let tile2_decor =
  group_pictures
    ([ tile2; south ptA2 10.0 (text "A"); south ptB2 10.0 (text "B") ;
     ...
     @ (List.map  (center_picture bullet) [ ptA2; ptB2; ptC2; ptD2; ptE2; ptF2 ])) ;;
...
let tile4sk =
  make_sketch [Seg (lineAC @ lineCD @ (List.rev lineAD))] ;;
...
let rec sol = function
  | [] -> []
  | (((_ct, t) as a) :: l) -> if List.exists (fun x -> close_enough 0.1 t (snd x)) l
    then sol l
    else a :: sol l ;;
...
let generate_coltrans_group' trl n=
  if trl = [] then []
  else let k= Array.length (fst (List.hd trl))
    in set_of_list
