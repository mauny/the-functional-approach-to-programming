...
let thai_elements =
  ...
  in let [txt1; txt2; txt3; txt4]
    = List.map (fun s -> extend_picture_frame Top_ext 2.0
  ...
  translate_picture (100.0, 300.0)
    (group_pictures
       ( (board_pict
          ...
          ::List.map (fun p -> center_picture vertic_pict
                         (compute_center (Vertic, p))) c.vertics)
         @ List.map (fun p -> center_picture square_pict
...
let texts_on_arrows fnt color sl =
  let w = List.fold_left (fun x s -> let  w = text_width fnt s
...
let thai_picts =
  List.map (fun c -> draw_config (snd c)) (make_conf_seq ml [start]) ;;

let rec nfirst l n =
  if n = 0 then [] else (List.hd l) :: nfirst (List.tl l) (n-1) ;;
let rec sub_list l m n=
  if m = 0 then nfirst l (n+1)
  else sub_list (List.tl l) (m-1) (n-1) ;;

let make_row_picture = function
  | [] ->  make_blank_picture (0.0, 0.0)
  | (p :: pl) -> align_horizontally Align_Center
                   (p :: List.map (extend_picture_frame Left_ext  0.2) pl) ;;

let make_column_picture = function
  | [] ->  make_blank_picture (0.0, 0.0)
  | (p :: pl) -> align_vertically Align_Left
                   (p :: List.map (extend_picture_frame Top_ext 0.2) pl) ;;
...
let [_c1; _c2; _c3; _c4; c5; c6] = List.map (fun m -> app_move start m)
    [m1; m2; m3; m4; m5; m6] ;;
...
let [p1; p2; p3; p4] = List.map (draw_config % snd) [c1; c2; c3; c4] ;;
...
let debut_graphe_pict =
  scale_picture (0.1, 0.1)
    (graphGen [float "lineWidthCoef" 1.0]
       debut_graphe
       (List.combine ["c1"; "c2"; "c3"; "c4"] [p1; p2; p3; p4] )) ;;
